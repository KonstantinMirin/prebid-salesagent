...
- `tests/fixtures/adcp_storyboards_pinned/index.json` — vendored, offline snapshot of the pinned compliance tree's storyboard structure (paths, phases, gates); pin-coupled, refreshed via its `_refresh.py`. Guarded by `tests/unit/test_architecture_storyboard_binding.py`'s `test_fixture_index_version_matches_the_pin` and cross-checked against `docs/test-obligations/storyboard-checks.jsonl` by `tests/unit/test_architecture_storyboard_check_index_artifact_truth.py`
