...
# ── Assertion 2: the audit chain never types the pinned version ──────────────

# Scoped to the surface this epic owns. NOT repo-wide: 109 files carry the
# version, including 846 `@source ... ref=vX.Y.Z` provenance citations across 29
# .feature files whose whole purpose is pinning provenance. A repo-wide ban
# would need a ~100-entry allowlist, which is the hand-maintained set this
# repo's allowlist rule exists to prevent.
_VERSION_SCOPE = ("scripts/audit", "tests/storyboard")
_VERSION_SCOPE_PLAIN = (Path("tox.ini"), Path(".github/workflows/ci.yml"))


def _pinned_version() -> str:
    from scripts.audit import storyboard_spec

    return storyboard_spec.pinned_version(REPO_ROOT)
...
def test_audit_chain_never_types_the_pinned_version() -> None:
    """A published string that names the version must READ it, not spell it.

    Docstrings and comments are exempt: "69 of 121 storyboards at 3.1.1" records
    a measurement taken at a pin and is not a claim the code makes at run time.
    An evaluated string constant IS such a claim — storyboard_roadmap.render()
    once interpolated the version into its header and hardcoded it three lines
    later in the same list, so one report would have named two versions.
    """
    version = _pinned_version()
    offenders: list[str] = []

    for scope in _VERSION_SCOPE:
        for path in sorted((REPO_ROOT / scope).rglob("*.py")):
            ...
            for node in ast.walk(tree):
                if (
                    ...
                    and version in node.value
                    and id(node) not in skip
                ):
                    offenders.append(f"{path.relative_to(REPO_ROOT)}:{node.lineno}")

    for rel in _VERSION_SCOPE_PLAIN:
        for lineno, line in enumerate((REPO_ROOT / rel).read_text(encoding="utf-8").splitlines(), 1):
            if version in line and not line.strip().startswith("#"):
                offenders.append(f"{rel}:{lineno}")

    assert offenders == [], (
        f"these sites spell the pinned version {version!r} instead of reading it from "
        "storyboard_spec.pinned_version(). On a repin each one publishes a claim about a "
        "version the code is no longer on:\n  " + "\n  ".join(offenders)
    )


# ── Meta-tests: the matchers must see what they claim to see ─────────────────
#
# Without these, the guard above asserts a property of the tree and nothing
# asserts a property of the guard. That asymmetry is how a half-blind matcher
# ships green: this matcher's first version caught two of eight spellings and
