"""Regression tests: identity resolution must have no ContextVar side effects.
...
class TestResolveIdentityPure:
    ...
    def test_no_set_current_tenant_in_resolve_identity(self):
        """_resolve_identity must not call set_current_tenant()."""
        source = (PROJECT_ROOT / "src" / "core" / "resolved_identity.py").read_text()
        tree = ast.parse(source)

        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef) and node.name == "_resolve_identity":
                func_source = ast.get_source_segment(source, node)
                assert func_source is not None
                assert "set_current_tenant" not in func_source, (
                    "_resolve_identity() calls set_current_tenant() -- "
                    "identity resolution must be pure (no ContextVar writes)"
                )
                return

        pytest.fail("_resolve_identity function not found")

    def test_no_get_current_tenant_fallback(self):
        """_resolve_identity must not fall back to get_current_tenant()."""
        source = (PROJECT_ROOT / "src" / "core" / "resolved_identity.py").read_text()
        tree = ast.parse(source)
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef) and node.name == "_resolve_identity":
                ...
                assert "get_current_tenant" not in func_source, (
                    "_resolve_identity() reads get_current_tenant() as fallback -- "
