...
Destination rewrites — a THIRD scan, with NO allowlist
------------------------------------------------------

Routing every request through the seam is not enough if code in front of the
...
the latent SSRF shape the #1589 follow-up removed (``_normalize_localhost_for_docker``
in ``protocol_webhook_service.py`` rewrote ``localhost`` to
``host.docker.internal`` before delivery, failing open on parse errors).

The third scan below flags URL reconstruction anywhere under ``src/``:
``urlunparse(...)`` / ``urlunsplit(...)`` calls and ``._replace(netloc=...)`` /
``._replace(scheme=...)`` (#1589). Nothing is exempt — not even the seam: address
policy there is delegated to ``adcp.signing``, which pins the resolved IP
without ever rewriting the URL, so a netloc rewrite inside the seam would be
...
def find_destination_rewrite_violations(tree: ast.Module) -> list[int]:
    ...
    Same ``(tree) -> list[int]`` detector shape as the other two scans, so the
    meta-tests feed all three the same way.
    """
...
class TestNoDestinationRewrite:
    ...
    #1589 follow-up and stays empty. Unlike the two scans above, even the seam
    is scanned — it pins addresses via ``adcp.signing`` without rewriting URLs.
    """

    @pytest.mark.arch_guard
    def test_no_destination_rewrites_anywhere(self):
        """Any urlunparse/urlunsplit or _replace(netloc/scheme=...) under src/ fails."""
        offenders = _scan_src(detector=find_destination_rewrite_violations, exempt=frozenset())
...
class TestDestinationRewriteDetector:
    ...
    def test_seam_is_scanned_and_clean(self):
        ...
        The two scans above exempt the seam by path because issuing egress is
        its job. Rewriting destinations is nobody's job — address policy lives
        in ``adcp.signing``, which pins IPs without touching the URL — so this
        asserts the seam passes the scan it is subject to.
        """
