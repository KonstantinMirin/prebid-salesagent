...
class TestRefusalTextIsAuthoredByThisModule:
    """No refusal sentence exists that ``egress/policy.py`` did not author.

    Covers salesagent-pldmk.6, Move 3.

    ``EgressPolicy.check_registration`` wraps its address computation in a bare
    ``except Exception as exc: raise AdCPBlockedUrlError(f"Invalid URL: {exc}")``.
    That interpolates a FOREIGN exception's own text into a buyer-facing refusal,
    in the one module that works hardest to stay cause-blind. The module docstring
    states why (invariant I5): one fixed message covers every address cause,
    because naming the cause turns a refusal into a topology probe.

    ``https://[oops/hook`` is the live demonstration -- ``urlsplit`` raises
    ``ValueError("Invalid IPv6 URL")`` and the arm ships that stdlib phrasing
    straight out to the buyer, via
    ``WebhookURLValidator.validate_webhook_url_registration`` ->
    ``reject_unsafe_webhook_registration_url``.

    The fix is to narrow the arm to ``except ValueError`` and emit the fixed
    sentence below. THE SENTENCE IS THE CONTRACT this test defines; the
    implementer must use it verbatim (recorded in salesagent-7r4vl.18's notes).
    It follows the house form already used by ``_registration_error``:
    "URL must use HTTPS scheme, got '...'", "URL must have a valid hostname",
    "URL resolves to a restricted range."
    """

    # A URL whose IPv6 bracket is unterminated. ``urlsplit`` raises ValueError
    # rather than returning a parse, so it reaches the wrapping arm.
    MALFORMED_URL = "https://[oops/hook"

    # The stdlib's own wording, which must not survive to a buyer.
    ...
    def test_refusal_does_not_carry_stdlib_phrasing(self):
        ...
        with pytest.raises(AdCPBlockedUrlError) as caught:
            EgressPolicy.check_registration(self.MALFORMED_URL)

        assert self.STDLIB_PHRASING not in str(caught.value), (
            f"the refusal carries stdlib phrasing this module did not author: {str(caught.value)!r}"
        )

    def test_refusal_is_the_fixed_sentence(self):
        """The refusal is exactly the sentence this module authored, and nothing else."""
        import pytest
        ...
        assert str(caught.value) == self.FIXED_SENTENCE

    def test_the_buyer_facing_wrapper_reports_the_same_fixed_sentence(self):
        """The sentence a buyer actually receives is the same one, not a variant.

        ``validate_webhook_url_registration`` is the ``(bool, str)`` wrapper the
        ingest gate calls, and its message is what ends up on the wire. Grading
        only the raised exception would leave the buyer-visible half unpinned.
        """
        is_valid, error = WebhookURLValidator.validate_webhook_url_registration(self.MALFORMED_URL)

        assert is_valid is False
        assert self.STDLIB_PHRASING not in error, f"the buyer-facing refusal carries stdlib phrasing: {error!r}"
        assert error == self.FIXED_SENTENCE
