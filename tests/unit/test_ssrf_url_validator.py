...
Covers:
- check_url_ssrf: core validator, still used by webhook delivery and property lists
- validate_agent_url: media_buy_create wrapper
- BLOCKED_HOSTNAMES: Docker-internal and cloud metadata hostname coverage

Admin ingest wiring is NOT covered here any more. The admin handlers no longer
call ``check_url_ssrf``; they apply egress policy through
``src.core.security.outbound_http.validate_url``, and the endpoint-level wiring
that used to live in this file is graded against a real database and the real
routes in ``tests/integration/test_admin_ingest_url_policy.py``.
"""
...
class TestValidateAgentUrl:
    ...
    network safety. Egress policy for user-supplied URLs is applied at the admin
    ingestion boundary via src.admin.utils.url_policy, which delegates to
    src.core.security.outbound_http.validate_url.
    """
