...
def then_rejected_validation_field(ctx: dict, field: str) -> None:
    ...
    Asserted through ``TransportResult.assert_wire_error`` — the one sanctioned
    error oracle (``test_architecture_bdd_wire_discipline``) — rather than a
    step-local envelope read: it reads ``wire_error_envelope`` specifically, so a
    synthesized envelope can never stand in for what the buyer received.
