...
class TestA2AWebhookPayloadTypes:
    ...
    async def test_completed_status_sends_task_payload(
        ...
        """
        # Auto-approval (so create_media_buy completes immediately) is the
        # baseline the autouse adapter_state_baseline fixture guarantees.
    ...
    async def test_submitted_status_sends_task_status_update_event(
        ...
        """
        # Opt IN to manual approval so create_media_buy returns submitted state.
        # adapter_config is SHARED ci-test tenant state; the autouse
        # adapter_state_baseline fixture resets it after this test, so there is
        # no restore to hand-roll (and none to forget).
...
class TestWebhookPayloadStructure:
    ...
    async def test_task_status_update_event_has_required_fields(
        ...
        # Opt IN to manual approval to get submitted status. adapter_config is
        # SHARED ci-test tenant state; the autouse adapter_state_baseline fixture
        # resets it after this test.
        set_live_adapter_behavior(live_server, manual_approval_required=True)
        a2a_url = f"{live_server['a2a']}/a2a"
