...
class TestProtocolWebhookWireFormat:
    ...
    def _send_and_capture(self, payload) -> dict[str, Any]:
        ...
        # runs in-process, so loopback is always the right callback host. The
        # ONE egress relaxation is the private-range hatch opened by
        # ``_open_egress_hatches``; the ADCP_TESTING carve-out that used to sit
        # here named ``url_validator``'s localhost allowance, which GH #1802
        # deleted — it relaxes nothing on the dial path and its presence would
        # read as a second, working hatch. The SSRF gate itself is never
        # patched — that would hide regressions.
        with run_loopback_capture_server(
