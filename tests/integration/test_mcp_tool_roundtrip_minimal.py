...
class TestMCPToolRoundtripMinimal:
    ...
    async def test_get_adcp_capabilities_accepts_version_envelope_fields(self, mcp_client):
        """get_adcp_capabilities must accept the AdCP 3.1.1 request-envelope fields.

        Every AdCP request carries adcp_version/adcp_major_version (AdcpVersionEnvelope)
        and an optional context object. The real conformance runner (SB-1b baseline,
        salesagent-xg5w.1) sends these on every get_adcp_capabilities call; our MCP
        wrapper's signature doesn't declare them, so FastMCP rejects the call before
        any capability data is returned.
        """
        result = await mcp_client.call_tool(
            "get_adcp_capabilities",
            {"adcp_version": "3.1", "adcp_major_version": 3, "context": {}},
        )

        assert result is not None
        content = result.structured_content if hasattr(result, "structured_content") else result
        assert "adcp" in content
    ...
    async def test_tool_accepts_version_envelope_fields(self, mcp_client, tool_name, extra_params):
        """Every AdCP tool wrapper must accept adcp_version/adcp_major_version (salesagent-xg5w.1).

        codebase-scan for xg5w.1 found the same missing-wrapper-param bug on 11 tools
        beyond get_adcp_capabilities. These 4 accept an all-optional payload, so a
        single parametrized test (rather than 4 hand-written near-duplicates) covers
        the version-envelope fields alone.
        """
        params = {"adcp_version": "3.1", "adcp_major_version": 3, **extra_params}
    ...
    @pytest.mark.parametrize(
        "tool_name,params",
        [
            ("sync_accounts", {"accounts": [{"account_id": "acct_test_1", "name": "Test Account"}]}),
            (
                "sync_creatives",
                {
                    "creatives": [
                        {
                            "creative_id": "test_creative_version_envelope",
                            "name": "Test Creative",
                            "format_id": {
            ...
            ("create_media_buy", {}),
            ("update_media_buy", {"media_buy_id": "mb_does_not_exist"}),
        ],
    )
    async def test_tool_does_not_reject_version_envelope_as_unexpected_kwarg(self, mcp_client, tool_name, params):
        """These tools need real business payloads for a full success test; this asserts
        only the specific failure mode xg5w.1 is about: FastMCP rejecting adcp_version/
        adcp_major_version as an unexpected keyword argument before the request ever
        reaches _impl. Any OTHER failure (missing business fields, not-found IDs) is
        acceptable here and is not what this regression test checks.
        """
        from fastmcp.exceptions import ToolError

        full_params = {"adcp_version": "3.1", "adcp_major_version": 3, **params}
        try:
            await mcp_client.call_tool(tool_name, full_params)
        except ToolError as e:
            message = str(e)
            assert "unexpected_keyword_argument" not in message or (
                "adcp_version" not in message and "adcp_major_version" not in message
            ), f"{tool_name} still rejects adcp_version/adcp_major_version as unexpected: {message}"
