"""Fitness function for the storyboard-conformance ledger (salesagent-qbac1.5, Lane E3).
...
``tests/storyboard/known_failures.txt`` — 75 entries, 43 ``mcp::`` + 32 ``a2a::``
...
declare all 75 entries stale on every developer machine, or skip and grade
...
def _ledger_entries() -> list[ledger.LedgerCheckId]:
    ...
    assert len(entries) == 75, f"expected the measured 75-entry storyboard ledger, loaded {len(entries)}"
...
def test_unconfigured_session_does_not_report_stale_entries(tmp_path: Path) -> None:
    ...
    id. Treating that as "74 of 75 entries are stale" is the failure mode that
