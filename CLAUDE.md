...
| internal_detail is an exception | `internal_detail=` (and `x.internal_detail =`) never takes a string or f-string, under `src/`, `scripts/` or `tests/`; the parameter is typed `BaseException \| None` and the boundary logs the cause once through the `from` chain | `.ast-grep/rules/internal-detail-is-an-exception.yml` + mypy on `src/` |
